...
public class NearbyWords implements SpellingSuggest {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (numSuggestions != 0 && queue.size() != 0) {
		    
		    
		    if (!visited.contains(queue.get(0)) && visited.size() < this.THRESHOLD) {
		        
		        visited.add(queue.get(0));
    		    List<String> nearBy = distanceOne(queue.remove(0), true);
    		    if (nearBy.size() <= numSuggestions) {
    		        while (!nearBy.isEmpty()) {
    		            queue.add(nearBy.get(0));
    		            retList.add(nearBy.remove(0));
    		            numSuggestions--;
    		        }
    		    } else {
    		        int count = numSuggestions;
    		        while (count != 0) {
    		            retList.add(nearBy.remove(0));
    		            count--;
    		        }
