...
public class BasicDocument extends Document 
{
	...
	public int getNumSyllables()
	{
	    ...
	    List<String> syllables1 = getTokens("[AaiIOouUYy]+");
	    List<String> words = getTokens("[a-zA-Z]+");
	    int syllables2 = 0;
	    int countOfE = 0;
	    for (int i = 0; i < words.size(); i++) {
	        ...
	        for (int j = 0; j < wordCharArray.length; j++) { 
	            ...
	            if ((wordCharArray[j] == 'e' || wordCharArray[j] == 'E') && j != (wordCharArray.length - 1) && previous != 'e' && 
	                    previous != 'E' && previous != 'a' && previous != 'A' && previous != 'i' && previous != 'I' &&
	                    previous != 'O' && previous != 'o' && previous != 'U' && previous != 'u' && previous != 'Y' &&
	                    previous != 'y') {
	                syllables2 = syllables2 + 1;
	                count++;
	            }
	            
	            if (wordCharArray[j] == 'a' || wordCharArray[j] == 'A' || wordCharArray[j] == 'i' || wordCharArray[j] == 'I' ||
                        wordCharArray[j] == 'O' || wordCharArray[j] == 'o' || wordCharArray[j] == 'U' || wordCharArray[j] == 'u' 
                        || wordCharArray[j] == 'Y' || wordCharArray[j] == 'y') {
	                count++;
	            }
	            
	            
	            if ((wordCharArray[j] == 'e' || wordCharArray[j] == 'E') && (j == (wordCharArray.length - 1)) && (count == 0)) {
	                syllables2 = syllables2 + 1;
	            }
	            previous = wordCharArray[j];
	        }
	        countOfE = syllables2 + countOfE;
	    }
        return syllables1.size() + countOfE;
	}
