...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		this.token = new String[tokens.size()];
	...
	public int getNumSentences() {
	    ...
	    for (int i = 0; i < token.length; i++) {
	        ...
	        if (isWord(token[i]) && i == token.length - 1) {
	            number++;
	        }
	    }
	    
	    this.numSentences = number;
		return number;
	}
	...
	public int getNumSyllables() {
	    ...
	    int number = 0;
        for (int i = 0; i < token.length; i++) {
            number += countSyllables(token[i]);
        }
        this.numSyllables = number;
        return number;
	}
